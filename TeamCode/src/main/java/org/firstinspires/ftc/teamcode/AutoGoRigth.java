...
@Autonomous(name = "AUTO_RIGHT", group = "Pushbot")
public class AutoGoRigth extends LinearOpMode {
    HardwarePushbot robot = new HardwarePushbot();

    private ElapsedTime runtime = new ElapsedTime();

    private int a = 0;
    public static double distanceEnd = 50;
    public static int TimeToGo = 5;
    public double X = 0.7;
...
//            man.runOpMode(robot);
        ...
        while (opModeIsActive() && (runtime.seconds() < TimeToGo) &&
                robot.sensorRange.getDistance(DistanceUnit.MM) < distanceEnd) {
            robot.angles = robot.imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYX, AngleUnit.DEGREES);
